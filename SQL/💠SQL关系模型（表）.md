...
1. 全局唯一GUID类型：使用一种全局唯一的字符串作为主键，类似`8f55d96b-8acc-4636-8cb8-76bf8abc2f57`。GUID算法通过网卡MAC地址、时间戳和随机数保证任意计算机在任意时间生成的字符串都是不同的，大部分编程语言都内置了GUID算法，可以自己预算出主键。

对于大部分应用来说，通常自增类型的主键就能满足需求。我们在`students`表中定义的主键也是`BIGINT NOT NULL AUTO_INCREMENT`类型。
如果使用INT自增类型，那么当一张表的记录数超过2147483647（约21亿）时，会达到上限而出错。使用BIGINT自增类型则可以最多约922亿亿条记录。

有事也可以采用联合主键，两条主键组合起来不重复就能够允许，但是这种情况一般不常用。
## 外键
外键通常是用来连接多张表的查询关系，比如一张表存放着学生基本身份信息，另一张表存放着学生的班级，如果想要知道某个身份信息的学生是哪个班级，这就需要在第一张表中确定一个东西去定位第二张表。这就是外键。
